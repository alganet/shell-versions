...
{
	case "$1" in
		0.2004-q)      echo b_2004-12-25 ;;
		0.2005-q)      echo b_2005-01-11 ;;
		0.2005-rminus) echo b_2005-09-16 ;;
		0.2006-r)      echo b_2006-01-24 ;;
		0.2006-rplus)  echo b_2006-06-16 ;;
		0.2006-s)      echo b_2006-12-15 ;;
...
shvr_build_ksh ()
{
	...
	# The pre-2007 history fork derives OPEN_MAX and OPEN_MAX_CEIL from the soft
	# RLIMIT_NOFILE (getdtablesize) at build time and bakes them into ksh's getconf
	# table AND into compile-time init constants. The soft limit is host-specific
	# (CI amd64 = 65536; CI arm64 and this Docker VM = 1048576), so an unpinned build
	# is not reproducible across machines. Lowering the soft limit to a fixed 65536
	# on EVERY arch forces every baked fd-limit copy to 65536: soft <= hard and the
	# hard limit is always >= 65536, so any host (CI, this VM, a contributor's box)
	# reproduces the same bytes. Newer forks compute these differently and are
	# unaffected. (Must be unconditional: gating it to amd64 left arm64 baking the
	# host's 1048576, which only matched whichever runner happened to share it.)
...
EOF
	...
	then
		...
		then
			# Pre-2007 history snapshots (0.2004-q .. 0.2006-s): the AST iffe
			# math-library probe fails against modern glibc, so -lm is dropped from
			# the ksh link and the libm references in libshell (pow/cos/floor/
			# strdata's local_*l math wrappers) are left undefined at link. Wrap
			# gcc-12 to always append -lm; it is ignored on compile-only (-c)
			# invocations and resolves the math symbols on the final link. Gated to
			# release year < 2007 (the 93s+ snapshot, 0.2007-s, is the first that
			# links libm on its own) so the >=2007 history builds (which have
			# committed checksums) keep using bare gcc-12 and stay byte-identical.
			ksh_cc=gcc-12
			ksh_year="${version#0.}"; ksh_year="${ksh_year%%-*}"
			if test "${ksh_year:-0}" -lt 2007
			then
				mkdir -p "${build_srcdir}/.shvr_bins"
				printf '#!/bin/sh\nexec gcc-12 "$@" -lm\n' > "${build_srcdir}/.shvr_bins/cc-lm"
				chmod +x "${build_srcdir}/.shvr_bins/cc-lm"
				ksh_cc="${build_srcdir}/.shvr_bins/cc-lm"
			fi
			bin/package make CC="$ksh_cc"
